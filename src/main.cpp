...
#define VBAT_MV_PER_LSB (0.439453126F)
        ...
        case eBT_liIon:
            ...
            if (isCharging)
            {
                if (volt <= LI_CHG_LV1)
                    ...
                    value =
                        70; // ここからは定電圧領域になるので電圧じゃほとんどわからない
            }
            else
            {
                if (volt <= LI_LV1)
...
void pin_init_and_button_attach(void)
{
    ...
    for (int i = 0; i < KEYSNUM; i++)
    {
        ...
        tactsw[i]->setDebounceMs(KEYDEBOUNCE);
...
bool battery_isCharging() { return digitalRead(PIN_INVCHG) == LOW; }
